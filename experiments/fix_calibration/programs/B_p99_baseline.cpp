...
static fxe::candidate_provenance make_provenance(const char* mode) {
    return fxe::candidate_provenance{
        .program_id = "B", .mode = mode,
        .target_description = "raw p99", .extractor_name = "onehot_extractor",
...
int main() {
    ...
    std::printf("  Rebuild fix_experiment_B_verify for Phase 2.\n");
    return 0;
}
...
int main() {
    ...
    fxe::compiled_measurer_dual<fxe::rdtsc_adapter, fxe::training_pool, B_candidates>
        measurer{adapter};
    auto discovery = fxe::run_discovery<fxe::onehot_extractor>(
        make_params(), measurer, target_fn());
    fxe::verify_candidates_match(discovery, B_candidates);
