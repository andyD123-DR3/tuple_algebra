// experiments/fix_calibration/programs/B_p99_baseline.cpp
//
// Program B: p99 baseline experiment.
//
// Question: "How much harder is p99 than p50?"
// Target:   raw p99 (nanoseconds)
// Encoding: onehot_extractor (36 features)
//
// Build targets (per-program, no global phase switch):
//
//   fix_experiment_B          Mock or Phase 1 (discovery)
//   fix_experiment_B_verify   Phase 2 (verification)
//
...
//     Trains SVR, runs beam search, writes generated/B_candidates.h,
//     generated/B_predicted_scores.h, and generated/B_reference_panel.h
//     to the source tree.
//
//   CTDP_PHASE2:
...
//     forecast accuracy (against p99) and cross-run reference panel drift.
...
#ifdef CTDP_PHASE2
#include "B_candidates.h"
#include "B_predicted_scores.h"
#include "B_reference_panel.h"
#endif
...
static fxe::experiment_params make_params() {
    return fxe::experiment_params{
        .program_id         = "B",
        .program_name       = "B_p99_baseline",
        .target_description = "raw p99",
        .target_units       = "ns",
    };
}

static auto target_fn() {
    return [](const fxe::measurement_result& r) { return r.p99_ns; };
}
    ...
    const char* array_name)
{
    ...
    os << "#ifndef CTDP_FIX_EXPERIMENT_B_PREDICTED_SCORES_H\n";
    os << "#define CTDP_FIX_EXPERIMENT_B_PREDICTED_SCORES_H\n\n";
    os << "#include <array>\n\n";
    ...
    const char* array_name)
{
    ...
    os << "#ifndef CTDP_FIX_EXPERIMENT_B_REFERENCE_PANEL_H\n";
    os << "#define CTDP_FIX_EXPERIMENT_B_REFERENCE_PANEL_H\n\n";
    os << "#include <array>\n\n";
    os << "namespace ctdp::fix_experiment::generated {\n";
    os << "inline constexpr std::array<double, "
...
// -- Forecast accuracy (p99-targeted) -----------------------------
//
// B's training target is raw p99, so forecast error is computed
// against measured_p99_ns.  The shared run_verification() picks
// best_measured_index by p50, which is wrong for B's target.
// We compute the p99-based best-measured index locally.
    ...
    const fxe::ExperimentReport& report)
{
    ...
    for (const auto& c : report.candidates) {
        ...
        // B targets p99: compare prediction against measured p99.
        const double err     = c.predicted_target - c.measured_p99_ns;
        const double abs_err = std::abs(err);
        abs_sum += abs_err;
        sq_sum  += err * err;
        ++n;
        if (c.measured_p99_ns > 0.0)
            apes.push_back(100.0 * abs_err / c.measured_p99_ns);
    }
...
static void print_forecast_summary(const forecast_summary& s) {
    std::printf(
        "\nForecast accuracy on verified shortlist (target: p99):\n"
        "  MAE                          : %.3f ns\n"
        "  RMSE                         : %.3f ns\n"
        "  MAPE                         : %.3f %%\n"
        "  Median APE                   : %.3f %%\n"
...
int main() {
    std::printf("Program B - p99 baseline (MOCK MODE)\n\n");
...
int main() {
    std::printf("Program B - p99 baseline (Phase 1: discovery)\n\n");
    ...
    // Persist Phase 1 reference panel for cross-run drift comparison.
    {
        ...
        write_reference_panel_header(
            ref_path, ref_p50s, "B_reference_panel");
    ...
    fxe::write_candidate_header(header_path, discovery.candidates,
        "B_candidates", make_provenance("rdtsc"));
    ...
    auto pred_path = std::filesystem::path(CTDP_EXPERIMENT_GENERATED_DIR)
        / "B_predicted_scores.h";
    write_predicted_scores_header(
        pred_path, discovery.predicted_scores, "B_predicted_scores");
    std::printf("  -> %s written (%zu scores)\n",
        pred_path.string().c_str(), discovery.predicted_scores.size());

    std::printf("\n  Rebuild fix_experiment_B_verify for Phase 2.\n");
...
int main() {
    std::printf("Program B - p99 baseline (Phase 2: verification)\n\n");

    auto mconfig = fxe::default_measurement_config();
    using namespace ctdp::fix_experiment::generated;

    // Compile-time check: generated artifacts must be aligned.
    static_assert(B_predicted_scores.size() == B_candidates.size(),
        "B_predicted_scores must match B_candidates size");
    static_assert(B_reference_panel.size() == fxe::num_baselines,
        "B_reference_panel must match baseline count");
    ...
    auto phase1_ref = load_phase1_reference(
        B_reference_panel.data(), B_reference_panel.size());
    print_reference_summary(
    ...
    if (rediscovery.candidates.size() != B_candidates.size()) {
        exact_match = false;
    } else {
        for (std::size_t i = 0; i < B_candidates.size(); ++i) {
            if (rediscovery.candidates[i] != B_candidates[i]) {
                exact_match = false;
                std::printf("  Warning: rediscovery mismatch at index %zu: "
                    ...
                    fix::config_to_string(B_candidates[i]).c_str());
                break;
            }
        }
    }
    ...
    // evaluation pool.  Dispatch table covers B_candidates (Pool1) and
    ...
    compiled_discovery.predicted_scores.assign(
        B_predicted_scores.begin(), B_predicted_scores.end());
    ...
    fxe::compiled_measurer_dual<
        fxe::rdtsc_adapter, B_candidates, fxe::baseline_configs>
    ...
    std::printf(
        ...
        "  the B-specific forecast summary after the table uses the correct p99 target.\n");
