...
//     Trains SVR, runs beam search, writes generated/C_candidates.h,
//     generated/C_predicted_scores.h, and generated/C_reference_panel.h
//     to the source tree.
//
//   CTDP_PHASE2:
...
#include "C_predicted_scores.h"
#include "C_reference_panel.h"
#endif
    ...
    const char* array_name)
{
    ...
    os << "#ifndef CTDP_FIX_EXPERIMENT_C_PREDICTED_SCORES_H\n";
    os << "#define CTDP_FIX_EXPERIMENT_C_PREDICTED_SCORES_H\n\n";
    os << "#include <array>\n\n";
    ...
    const char* array_name)
{
    ...
    os << "#ifndef CTDP_FIX_EXPERIMENT_C_REFERENCE_PANEL_H\n";
    os << "#define CTDP_FIX_EXPERIMENT_C_REFERENCE_PANEL_H\n\n";
    os << "#include <array>\n\n";
    os << "namespace ctdp::fix_experiment::generated {\n";
    os << "inline constexpr std::array<double, "
...
int main() {
    ...
    // Persist Phase 1 reference panel for cross-run drift comparison.
    {
        ...
        write_reference_panel_header(
            ref_path, ref_p50s, "C_reference_panel");
    ...
    auto pred_path = std::filesystem::path(CTDP_EXPERIMENT_GENERATED_DIR)
        / "C_predicted_scores.h";
    write_predicted_scores_header(
        pred_path, discovery.predicted_scores, "C_predicted_scores");
...
int main() {
    ...
    static_assert(C_predicted_scores.size() == C_candidates.size(),
        "C_predicted_scores must match C_candidates size");
    static_assert(C_reference_panel.size() == fxe::num_baselines,
        "C_reference_panel must match baseline count");
    ...
    auto phase1_ref = load_phase1_reference(
        C_reference_panel.data(), C_reference_panel.size());
    print_reference_summary(
    ...
    if (rediscovery.candidates.size() != C_candidates.size()) {
        exact_match = false;
    } else {
        for (std::size_t i = 0; i < C_candidates.size(); ++i) {
            if (rediscovery.candidates[i] != C_candidates[i]) {
                exact_match = false;
                std::printf("  Warning: rediscovery mismatch at index %zu: "
                    ...
                    fix::config_to_string(C_candidates[i]).c_str());
                break;
            }
        }
    }
    ...
    // evaluation pool.  Dispatch table covers C_candidates (Pool1) and
    ...
    compiled_discovery.candidates.assign(
        C_candidates.begin(), C_candidates.end());
    compiled_discovery.predicted_scores.assign(
        C_predicted_scores.begin(), C_predicted_scores.end());
    ...
    fxe::compiled_measurer_dual<
        fxe::rdtsc_adapter, C_candidates, fxe::baseline_configs>
    ...
    std::printf(
        ...
        "  the C-specific forecast summary after the table uses the correct log(p99) target.\n");
