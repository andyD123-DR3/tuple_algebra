...
//
// Program D: log-transformed p99 experiment.
//
...
//     Trains SVR, runs beam search, writes generated/D_candidates.h,
//     generated/D_predicted_scores.h, and generated/D_reference_panel.h
//     to the source tree.
//
//   CTDP_PHASE2:
...
#include "D_predicted_scores.h"
#include "D_reference_panel.h"
#endif
...
static fxe::candidate_provenance make_provenance(const char* mode) {
    return fxe::candidate_provenance{
        ...
        .mode               = mode,
        .target_description = "log(p99)",
        .extractor_name     = "count_extractor",
    ...
    const char* array_name)
{
    ...
    os << "#ifndef CTDP_FIX_EXPERIMENT_D_PREDICTED_SCORES_H\n";
    os << "#define CTDP_FIX_EXPERIMENT_D_PREDICTED_SCORES_H\n\n";
    os << "#include <array>\n\n";
    ...
    const char* array_name)
{
    ...
    os << "#ifndef CTDP_FIX_EXPERIMENT_D_REFERENCE_PANEL_H\n";
    os << "#define CTDP_FIX_EXPERIMENT_D_REFERENCE_PANEL_H\n\n";
    os << "#include <array>\n\n";
    os << "namespace ctdp::fix_experiment::generated {\n";
    os << "inline constexpr std::array<double, "
...
int main() {
    ...
    // Persist Phase 1 reference panel for cross-run drift comparison.
    {
        ...
        write_reference_panel_header(
            ref_path, ref_p50s, "D_reference_panel");
    ...
    auto pred_path = std::filesystem::path(CTDP_EXPERIMENT_GENERATED_DIR)
        / "D_predicted_scores.h";
    write_predicted_scores_header(
        pred_path, discovery.predicted_scores, "D_predicted_scores");
...
int main() {
    ...
    static_assert(D_predicted_scores.size() == D_candidates.size(),
        "D_predicted_scores must match D_candidates size");
    static_assert(D_reference_panel.size() == fxe::num_baselines,
        "D_reference_panel must match baseline count");
    ...
    auto phase1_ref = load_phase1_reference(
        D_reference_panel.data(), D_reference_panel.size());
    print_reference_summary(
    ...
    auto rediscovery = fxe::run_discovery<fxe::count_extractor>(
        make_params(), train_measurer, target_fn());

    bool exact_match = true;
    if (rediscovery.candidates.size() != D_candidates.size()) {
        exact_match = false;
    } else {
        for (std::size_t i = 0; i < D_candidates.size(); ++i) {
            if (rediscovery.candidates[i] != D_candidates[i]) {
                exact_match = false;
                std::printf("  Warning: rediscovery mismatch at index %zu: "
                    ...
                    fix::config_to_string(D_candidates[i]).c_str());
                break;
            }
        }
    }
    ...
    // evaluation pool.  Dispatch table covers D_candidates (Pool1) and
    ...
    compiled_discovery.candidates.assign(
        D_candidates.begin(), D_candidates.end());
    compiled_discovery.predicted_scores.assign(
        D_predicted_scores.begin(), D_predicted_scores.end());
    ...
    fxe::compiled_measurer_dual<
        fxe::rdtsc_adapter, D_candidates, fxe::baseline_configs>
    ...
    std::printf(
        ...
        "  the D-specific forecast summary after the table uses the correct log(p99) target.\n");
